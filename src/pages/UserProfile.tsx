...
const UserProfile: React.FC = () => {
  ...
  const confirmLogout = () => { logout(); navigate('/login'); setShowLogoutDialog(false); };
  const handleDeleteAccount = () => setShowDeleteDialog(true);
  const confirmDeleteAccount = async () => {
    const success = await deleteAccount();
    if (success) { setShowDeleteDialog(false); navigate('/login?deleted=true'); }
  };
    ...
    <div className={`min-h-screen ${currentBackgroundClass} flex flex-col`}>
      <div className="flex-1 overflow-auto">
        <div className="container mx-auto max-w-2xl px-3 sm:px-4 py-4 pb-6">
          <Card className="mb-4 bg-white/80 backdrop-blur-sm">
            ...
            <CardContent className="space-y-4 px-4 sm:px-6">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="font-medium text-sm">Favorite Mosques: {favorites.length}</span>
              </div>
              ...
              <div className="space-y-3">
                {isUserAdmin && (
                  <div className="space-y-2">
                    ...
                    <Button onClick={() => setShowAdminPanel(!showAdminPanel)} variant="outline" className="w-full text-sm" size="sm">
                ...
                <Button onClick={handleLogout} variant="outline" className="w-full text-sm" size="sm">Logout</Button>
                <Button onClick={handleDeleteAccount} variant="destructive" className="w-full text-sm bg-red-600 hover:bg-red-700" size="sm">
          ...
          {showAdminPanel && isUserAdmin && <div className="mb-4"><AdminPanel /></div>}
        </div>
      </div>
      
      <LogoutConfirmationDialog isOpen={showLogoutDialog} onClose={() => setShowLogoutDialog(false)} onConfirm={confirmLogout} />
      <DeleteAccountDialog isOpen={showDeleteDialog} onClose={() => setShowDeleteDialog(false)} onConfirm={confirmDeleteAccount} isLoading={isLoading} />
      <BottomBar />
    </div>
  );
};
