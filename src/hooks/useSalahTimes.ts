...
export const useSalahTimes = (selectedDate: Date) => {
  ...
  useEffect(() => {
    const fetchSalahTimes = async () => {
      ...
      try {
        ...
        // Data source: https://prayer-times.muslimpro.com/en/find?country_code=IN&country_name=India&city_name=Bareilly&coordinates=28.3670355,79.4304381
        const dateObj = new Date(selectedDate);
        const month = dateObj.getMonth();
        const day = dateObj.getDate();
        
        // More accurate Bareilly prayer times from Muslim Pro
        let times: SalahTime;
  ...
  const isPrayerPassed = (prayerTime: string): boolean => {
    ...
    if (selectedDate < now && !isSameDay(selectedDate, now)) {
      return true;
    }
